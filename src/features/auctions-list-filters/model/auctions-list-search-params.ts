...
const AUCTION_TYPE_VALUES = ['Request', 'Up', 'Down', 'FixPrice'] as const satisfies readonly Exclude<TAuctionType, 'Unknown'>[]
...
export const auctionsListSearchParamsSchema = z.object({
  page: z.preprocess(
    normalizeOptionalNumberSearchParam,
  ...
  status: z.preprocess(
    normalizeArraySearchParam,
    z.array(z.enum(TRADING_STATUS_VALUES)).catch([]),
  ),
  ...
  auc_type: z.preprocess(
    normalizeArraySearchParam,
    z.array(z.enum(AUCTION_TYPE_VALUES)).catch([]),
  ),
...
function removeEmptyAuctionListRequestParams(
