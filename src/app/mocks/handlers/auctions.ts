...
import {
  readAuctionListRequest,
  readSetBetRequest,
} from '@/app/mocks/helpers/read-auction-requests'
import {
  createBetItem,
...
export const auctionHandlers = [
  http.post('/api/v1/auctions/list', async ({ request }) => {
    const body = await readAuctionListRequest(request)
    ...
    const data = filteredItems
      ...
      .map(mapAuctionStoreItemToListItem)
  ...
  http.post('/api/v1/auctions/:auctionUuid/bets', async ({ params, request }) => {
    ...
    const body = await readSetBetRequest(request)
