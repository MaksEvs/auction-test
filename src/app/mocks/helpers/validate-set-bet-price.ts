...
export function validateSetBetPrice(
  ...
  if (!Number.isFinite(price) || price <= 0) {
    return createPriceValidationError('Цена ставки должна быть больше 0.', 'greater_than');
  }

  if ((auction.main.auc_type === 'Down' || auction.main.auc_type === 'Up') && available === null) {
    return createPriceValidationError(
  ...
  if (min !== null && price < min) {
    return createPriceValidationError(`Цена ставки не может быть меньше ${min}.`, 'minimum');
  }

  if (max !== null && price > max) {
    return createPriceValidationError(`Цена ставки не может быть больше ${max}.`, 'maximum');
  }

  if (auction.main.auc_type === 'Down' && available !== null && price > available) {
    return createPriceValidationError(
  ...
  if (auction.main.auc_type === 'Up' && available !== null && price < available) {
    return createPriceValidationError(
...
function createPriceValidationError(message: string, code: string): IValidationError {
  return {
    field: 'price',
    message,
    code,
