...
export function AuctionBetsPage() {
  ...
  if (isApiNotFoundError(auctionQuery.error) || isApiNotFoundError(betsQuery.error)) {
    return <NotFoundPage />;
  }
  ...
  if (auctionQuery.isPending) {
    ...
    content = <Alert severity="error">Не удалось загрузить информацию об аукционе.</Alert>;
  } else if (isHistoryHidden) {
    content = (
      <Alert severity="warning">Организатор ограничил доступ к ставкам этого аукциона.</Alert>
    );
    ...
    content = <Alert severity="error">Не удалось загрузить ставки.</Alert>;
  } else if (betsQuery.data.bets.length === 0) {
    content = <Alert severity="info">В этом аукционе пока нет ставок.</Alert>;
  } else {
    const participantCount = new Set(betsQuery.data.bets.map((bet) => bet.subscriber_id)).size;
  ...
  return (
    <AuctionBetsPageContainerStyled maxWidth="xl">
      <AuctionBetsBackButtonStyled onClick={handleAuctionOpen} variant="outlined">
        К аукциону
      </AuctionBetsBackButtonStyled>
